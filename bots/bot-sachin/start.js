 ...
 * ============================================================================
 * This file is placed in each bot's folder (e.g., bots/bot-delhi/start.js)
...
const BOT_DIR = __dirname; // e.g., bots/bot-delhi/
...
function printOperationalSummary() {
  ...
  log.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  log.info(`🛡️  ANTI-BAN HARDENING`);
  log.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  log.info(`   ✅ B1: Reconnect age-gate (${config.reconnect.strictAgeMs / 1000}s strict for ${config.reconnect.strictWindowDuration / 1000}s)`);
  log.info(`   ✅ B2: Replay ID protection (${config.deduplication.maxReplayIds} rolling IDs)`);
  log.info(`   ✅ C1: Fingerprint batch cleanup (trim to ${config.deduplication.cleanupTargetRatio * 100}% on overflow)`);
  log.info(`   ✅ C2: Disk-backed persistence (${config.deduplication.saveDebounceMs / 1000}s debounce)`);
  log.info(`   ✅ A4: Settling delay (${config.reconnect.settlingMin / 1000}-${config.reconnect.settlingMax / 1000}s on first message)`);
  log.info(`   ✅ A1: Length-scaled typing (${config.humanBehavior.typingMin / 1000}-${config.humanBehavior.typingMax / 1000}s)`);
  log.info(`   ✅ A5: Weighted between-group gaps (${config.humanBehavior.betweenWeight * 100}% bias low)`);
  log.info(`   ✅ A3: Fisher-Yates shuffle (random target order every send)`);
  log.info(`   ✅ Circuit breaker (${config.circuitBreaker.maxFailures} failures = ${config.circuitBreaker.breakDuration / 1000}s cooldown)`);
  log.info(`   ✅ Per-group cooldown (${config.deduplication.sendCooldown / 1000}s minimum between sends)`);
  log.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
