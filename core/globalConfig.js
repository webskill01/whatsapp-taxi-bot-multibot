...
// Single source of truth for the volatile block/ignore data file path.
const BLOCKED_DATA_PATH = join(__dirname, "blocked-data.json");
...
function loadBlockedData() {
  const filePath = BLOCKED_DATA_PATH;
  let data;
  try {
...
try {
  watchFile(BLOCKED_DATA_PATH, { interval: 1000 }, (curr, prev) => {
    ...
    _reloadDebounce = setTimeout(() => {
      try {
        const fresh = JSON.parse(readFileSync(BLOCKED_DATA_PATH, "utf8"));
